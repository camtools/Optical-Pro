...
const S={
  ...
  overlays:{frame:true,frameLabel:false,safe:false,grid:false,cross:false,info:true,horizon:false}
};
...
async function startRecording(){
  try{
    ...
    mediaRecorder.onstop=()=>{
      ...
      generateVideoThumb(blobUrl).then(thumb=>{entry.thumbUrl=thumb;renderGalleryGrid();});
      videos.push(entry);
      updateGalleryThumb();
...
function draw(){
  ...
  if(S.overlays.info){
    ...
    const st=`${m.w}×${m.h}mm`;
    ctx.fillText(st,fX+fW-ctx.measureText(st).width-pad,fY+fH-pad);
  }
...
function captureScreenshot(){
  // Disable info overlay temporarily so corners are clean in the frame
  const wasInfo=S.overlays.info;
  S.overlays.info=false;
  requestAnimationFrame(()=>requestAnimationFrame(()=>{doCapture();S.overlays.info=wasInfo;}));
}
...
function doCapture(){
  ...
  if(screenshots.length>=MAX_SHOTS)screenshots.shift();
...
function buildOverlayChips(){
  const defs=[{k:'frame',l:'Frame'},{k:'frameLabel',l:'Frame Line'},{k:'safe',l:'Safe Area'},{k:'grid',l:'Grid 3×3'},{k:'cross',l:'Crosshair'},{k:'info',l:'Info'},{k:'horizon',l:'Horizon'}];
...
(function(){
  ...
  setTimeout(()=>splash.classList.add('fade'),3100);
  setTimeout(()=>{
